...
CREATE OR REPLACE PROCEDURE FORECAST.SP_BATCH_SAVE_FORECASTS_CHAINS(
...
DECLARE
    ...
    missing_required_fields_ex EXCEPTION (-20093, 'Missing required field. Please provide market_code in each record.');
    missing_volume_fields_ex EXCEPTION (-20094, 'Missing required fields for volume update. Please provide variant_size_pack_id, forecast_year, month, and parent_chain_code when manual_case_equivalent_volume is present.');
    ...
    V_FORECAST_GENERATION_MONTH_DATE DATE;
BEGIN
    -- Get the valid forecast generation month date (used only for publication checking)
    -- NOTE: Chains table is a running table without FGMD field, but we still need to check
    -- if markets are published for the current forecast generation cycle
    SELECT FORECAST.UDF_GET_VALID_FORECAST_GENERATION_MONTH_DATE() INTO :V_FORECAST_GENERATION_MONTH_DATE;
    ...
    WHERE FORECAST.UDF_IS_DEPLETIONS_FORECAST_PUBLISHED(s.market_code, :V_FORECAST_GENERATION_MONTH_DATE);
    ...
    SELECT COUNT(*) INTO :V_TEMP_COUNT FROM (
        ...
        GROUP BY
            ...
            f.value:parent_chain_code::VARCHAR,
    ...
    WHERE f.value:manual_case_equivalent_volume IS NOT NULL
      AND (
        ...
        OR (f.value:parent_chain_code::VARCHAR IS NULL OR f.value:parent_chain_code::VARCHAR = '')
    ...
    BEGIN
        ...
        MERGE INTO FORECAST.MANUAL_INPUT_DEPLETIONS_FORECAST_CHAINS AS target
        USING (
            WITH unnested_forecasts AS (
                SELECT
                    f.value:market_code::VARCHAR AS market_code,
                    f.value:customer_id::VARCHAR AS customer_id,
                    f.value:parent_chain_code::VARCHAR AS parent_chain_code,
            ...
            JOIN FORECAST.DISTRIBUTOR_ALLOCATION_BY_MARKET_SIZE_PACK_CHAINS d
                ON uf.market_code = d.market_code
                AND uf.parent_chain_code = d.parent_chain_code
        ...
        ON (
            ...
            AND target.PARENT_CHAIN_CODE = source.parent_chain_code
        ...
        WHEN NOT MATCHED THEN
            INSERT (
                ...
                PARENT_CHAIN_NAME, PARENT_CHAIN_CODE, BRAND, BRAND_ID, VARIANT, VARIANT_ID, 
                ...
                source.parent_chain_name, source.parent_chain_code, source.brand, source.brand_id, 
                source.variant, source.variant_id, source.variant_size_pack_desc, source.variant_size_pack_id, 
                source.forecast_year, source.month, source.allocated_volume, :P_USER_ID, :P_FORECAST_STATUS, 
                source.comment, CURRENT_TIMESTAMP(), 1
            );

        V_MERGE_COUNT := SQLROWCOUNT;

        -- Step 2: Create new versions for all records that were just updated or inserted by the MERGE.
        INSERT INTO FORECAST.MANUAL_INPUT_DEPLETIONS_FORECAST_CHAINS_VERSIONS (
        ...
        SELECT
            ...
            t.MARKET_NAME, t.MARKET_CODE, t.DISTRIBUTOR_NAME, t.DISTRIBUTOR_ID, 
            t.PARENT_CHAIN_NAME, t.PARENT_CHAIN_CODE, t.BRAND, t.BRAND_ID,
            t.VARIANT, t.VARIANT_ID, t.VARIANT_SIZE_PACK_DESC, t.VARIANT_SIZE_PACK_ID, 
            t.FORECAST_YEAR, t.MONTH, t.MANUAL_CASE_EQUIVALENT_VOLUME,
            t.UPDATED_BY_USER_ID, t.FORECAST_STATUS, t.COMMENT
        FROM FORECAST.MANUAL_INPUT_DEPLETIONS_FORECAST_CHAINS t
        JOIN (
        ...
        ON t.MARKET_CODE = s.market_code 
           AND t.PARENT_CHAIN_CODE = s.parent_chain_code
           ...
           AND t.MONTH = s.month
        WHERE NOT EXISTS ( -- Only insert a version if it doesn't already exist for that number
              SELECT 1 FROM FORECAST.MANUAL_INPUT_DEPLETIONS_FORECAST_CHAINS_VERSIONS v
        ...
        -- NOTE: Chains primary forecast method table also follows running table behavior
        -- (no forecast_generation_month_date field, represents current state)
        UPDATE FORECAST.DEPLETIONS_FORECAST_PRIMARY_FORECAST_METHOD_CHAINS AS target
        SET target.FORECAST_METHOD = f.value:forecast_method::VARCHAR
        FROM
            TABLE(FLATTEN(input => PARSE_JSON(:P_FORECASTS_JSON))) f
        WHERE
            target.MARKET_CODE = f.value:market_code::VARCHAR
            AND target.PARENT_CHAIN_CODE = f.value:parent_chain_code::VARCHAR
        ...
        RETURN 'SUCCESS: Processed batch for chains running table. Primary methods updated: ' || V_UPDATE_COUNT || '. Volume records merged: ' || V_MERGE_COUNT || '.';
    EXCEPTION
        WHEN OTHER THEN
            ROLLBACK;
...
COMMENT ON PROCEDURE FORECAST.SP_BATCH_SAVE_FORECASTS_CHAINS(VARCHAR, VARCHAR, VARCHAR) IS 
'CHAINS BATCH SAVE: Handles bulk updates to chains manual input table with running table behavior.
KEY DIFFERENCE: Chains manual input table has no FORECAST_GENERATION_MONTH_DATE field - represents current state only.
Performs publication checks against current FGMD but updates running table regardless of forecast generation cycle.'; 