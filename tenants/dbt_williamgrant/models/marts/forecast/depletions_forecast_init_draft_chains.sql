...
with source_data as (
  ...
  select
    ...
    distributor_name,
    distributor_id,
    vip_parent_chain_name as parent_chain_name,
    ...
    month,
    month_date,
    DATEADD(YEAR, 1, month_date) as forecast_ref_date,
    sum(coalesce(case_equivalent_quantity, 0)) as case_equivalent_depletions
  from
    {{ ref('rad_invoice_level_sales') }}
  where vip_parent_chain_code is not null and vip_parent_chain_code != '' and vip_parent_chain_code != '33333'
  {% if backfill_fgmd %}
  -- When backfilling, only include data up to the month before the backfill FGMD
  and month_date <= '{{ backfill_fgmd }}'::date
  {% endif %}
  group by all
),
...
unique_product_combinations_with_descriptives as (
  ...
  inner join unique_ids ui
    ...
    and src.parent_chain_code = ui.parent_chain_code
...
densified_data as (
  select
    ...
    upc.distributor_id,
    upc.parent_chain_name,
    upc.parent_chain_code,
    upc.brand,
    upc.brand_id,
    upc.variant,
    upc.variant_id,
  ...
  left join
    ...
    and upc.parent_chain_code = sd.parent_chain_code
  ...
  select
    ...
    sd.distributor_name,
    sd.distributor_id,
    sd.parent_chain_name,
    sd.parent_chain_code,
    sd.brand,
    sd.brand_id,
    sd.variant,
    sd.variant_id,
...
monthly_data_with_py as (
  ...
  left join
    ...
    and dd.parent_chain_code = py.parent_chain_code
...
rolling_sums as (
  select
    ...
    sum(case_equivalent_depletions) over (
      partition by market_code, distributor_id, parent_chain_code, variant_size_pack_id
      order by month_date
...
-- FGMD-only month-end projection at chain grain
fgmd_prediction as (
  select
    p.market_code,
    p.distributor_id,
  ...
  from {{ forecast__monthend_projection('chain', 'fgmd') }} p
),
...
trend_factors_unpivoted as (
  ...
  select
    market_name, market_code, distributor_name, distributor_id, parent_chain_name, parent_chain_code, brand, brand_id, variant, variant_id, variant_size_pack_desc, variant_size_pack_id,
    latest_complete_month_date,
...
forecast_rows as (
  select
    ...
    tfu.distributor_id,
    tfu.parent_chain_name,
    tfu.parent_chain_code,
    tfu.brand,
    tfu.brand_id,
    tfu.variant,
    tfu.variant_id,
    ...
    greatest(
      case
        when fmt.forecast_month_date = rd.forecast_generation_month_date then coalesce(fp.projected_case_equivalent_quantity, 0.0)
  ...
  cross join run_details rd
  left join source_data pya
    on tfu.market_code = pya.market_code
   and tfu.distributor_id = pya.distributor_id
   and tfu.parent_chain_code = pya.parent_chain_code
  ...
  left join rolling_sums rs
    on tfu.market_code = rs.market_code
   and tfu.distributor_id = rs.distributor_id
   and tfu.parent_chain_code = rs.parent_chain_code
  ...
  left join fgmd_prediction fp
   ...
   and fp.parent_chain_code = tfu.parent_chain_code
  ...
  where DATEDIFF(MONTH, rd.latest_complete_month_date, fmt.forecast_month_date) <= 12
),
...
actual_rows as (
  select
    ...
    cya.distributor_id,
    cya.parent_chain_name,
    cya.parent_chain_code,
    cya.brand,
    cya.brand_id,
    cya.variant,
    cya.variant_id,
    ...
    rs.py_6m_sum,
    rs.py_12m_sum
  from current_year_actuals cya
  cross join distinct_methods dm
  cross join run_details rd
  left join source_data pya
    on cya.market_code = pya.market_code
   and cya.distributor_id = pya.distributor_id
   and cya.parent_chain_code = pya.parent_chain_code
   and cya.variant_size_pack_id = pya.variant_size_pack_id
   and cya.month_date = pya.forecast_ref_date
  left join rolling_sums rs
    on cya.market_code = rs.market_code
   and cya.distributor_id = rs.distributor_id
   and cya.parent_chain_code = rs.parent_chain_code
...
planned_zero_rows as (
  select
    ...
    cu.distributor_name,
    cu.distributor_id,
    cu.parent_chain_name,
    cu.parent_chain_code,
    av.brand,
    av.brand_id,
    av.variant,
    av.variant_id,
    av.variant_size_pack_desc,
    av.variant_size_pack_id,
    fmt.forecast_year,
    fmt.month,
    fmt.forecast_month_date,
    mth.forecast_method,
  ...
  from (
    select distinct market_code, market_name, distributor_id, distributor_name, vip_parent_chain_code as parent_chain_code, vip_parent_chain_name as parent_chain_name
    from {{ ref('rad_invoice_level_sales') }}
    where vip_parent_chain_code is not null and vip_parent_chain_code != '' and vip_parent_chain_code != '33333'
  ) cu
  cross join future_months_template fmt
  cross join run_details rd
  join {{ source('master_data', 'apollo_variant_size_pack_tag') }} tag_planned on 1=1
  left join asm_vsp av on av.variant_size_pack_id = tag_planned.variant_size_pack_id
  join {{ ref('apollo_dist_master') }} adist
    on adist.distributor_id = cu.distributor_id
  cross join (
    select 'three_month' as forecast_method
    union all select 'six_month'
    union all select 'twelve_month'
  ...
  where lower(tag_planned.is_planned) = 'true'
    and adist.is_depletions_eligible = 1
    -- Note: NULL is_planned leads to no zero seeding; products appear only if actuals exist
    and (
      tag_planned.market_code_exclusions is null or array_size(tag_planned.market_code_exclusions) = 0
      or not array_contains(cu.market_code::variant, tag_planned.market_code_exclusions)
    )
    and (
      tag_planned.customer_id_exclusions is null or array_size(tag_planned.customer_id_exclusions) = 0
      or not array_contains(cu.distributor_id::variant, tag_planned.customer_id_exclusions)
    )
    and not exists (
      select 1
      from combined_data cd
      where cd.market_code = cu.market_code
        and cd.distributor_id = cu.distributor_id
        and cd.parent_chain_code = cu.parent_chain_code
        and cd.variant_size_pack_id = tag_planned.variant_size_pack_id
...
integrated_data as (
  select *, 'logic_driven' as data_source from combined_data
  union all
  select *, 'zero_seeded' as data_source from planned_zero_rows
)
...
select
  ...
  id.distributor_name,
  id.distributor_id,
  id.parent_chain_name,
  id.parent_chain_code,
  id.brand,
  id.brand_id,
  id.variant,
  id.variant_id,
